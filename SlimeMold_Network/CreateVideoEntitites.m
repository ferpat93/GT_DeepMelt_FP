...
FPS=3; % Frames per second of video
nF=numel(times); %Total number of frames to display

for i=3:numel(dishes)
    e = dishes(i);
    mask = uint8(createCircleMask(ROI(e,3))); 
    mask3 = repelem(mask,1,1,3);
    position = [1 1];
    treat = TF.Treatment{e};
    centroids = [];
    
    % Video Parameters
    filepathE = fullfile(pwd,strcat('Video_Entities_',treat));
    ...
    for ti=1:numel(times)
        ...
        [BW,I,centroids]= getBinary(path,ROI(e,:),centroids);
        [labeled_clusters,skeleton]=GetClusters_Skeleton(BW);

        B = superposedImage(I,BW,skeleton,labeled_clusters);
        B(~mask3)= 255;
        B = uint8(insertText(B,position,strcat(treat,' - t: ',num2str(time/60,3),'h'),'BoxOpacity',1,'BoxColor','w','FontSize',40));
...
function [BW,I,centroids] = getBinary(path,ROI,centroids)
