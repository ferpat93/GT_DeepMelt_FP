...
%% INPUT VARIABLES
NX=10; %Nodes on X direction
NY=10; %Nodes on Y direction
D=1; %Lattice distance (side of equilateral triangle)

mp=145;%e-6m; %Mean of pore diameter %values in nanometers
%sp=mp*0.000001;
sp=35;%e-6m; %Std deviation of pore diameter

mr=0.480;%e-6; %Mean of edge radii
%sr=mr*0.25;
sr=0.120;%e-6; %Std deviation of edge radii

n=1;%e-3; %Viscosity of the fluid
kf=pi()/(8*n); % Constant of flow

gplot=1; %Generate plot? true=1 false=0
%% SET EDGES
...
for k=1:length(edges(:,1))
    edges(k,1)=norminv(rand(),mr,sr); %set normally distributed width of edge  
...
for k=1:NX %Iterates over X
    ...
    nodes(f,k,1)=norminv(rand(),mp,sp); %set normally distributed size of pore
