...
%%
[~,I]=sort(edges(:,3),'descend'); %sorts matrix of edges by highest to minimum radius
...
lc=cellfun(@length,Clusters);
figure(1)
histogram(lc)

%figure(3)
%scatter(edges(Clusters{1},1),edges(Clusters{1},2))
...
for j=1:length(index) % Runs on every edge
    ...
    set(L1,'LineWidth',(4*edges(index(j),3)/k+0.001)) %Sets linewidth according to r value
end
