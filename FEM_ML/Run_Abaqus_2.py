...
def f_CreateModel(H,Dc,Hm,Wm,d,E,v,phi,psi,c,Name): # Creates Part from sketch
    ...
    vertex_ULWall = Vertices.getSequenceFromMask(mask=('[#10 ]', ), )
    Assembly.Set(edges=edges_ULWall, xVertices=vertex_ULWall, name='Set-ULWall')
    edges_LLWall = Edges.getSequenceFromMask(mask=('[#1 ]', ), )
    vertex_LLWall = Vertices.getSequenceFromMask(mask=('[#2 ]', ), )
    Assembly.Set(edges=edges_LLWall, xVertices=vertex_LLWall, name='Set-LLWall')
...
#     Model.XsymmBC(name='BC_ULWall', createStepName='Initial',region=Assembly.sets['Set-ULWall'],localCsys=None)
    Model.YsymmBC(name='BC_LLWall', createStepName='Initial',region=Assembly.sets['Set-LLWall'],localCsys=polar)
    Model.YsymmBC(name='BC_ULWall', createStepName='Initial',region=Assembly.sets['Set-ULWall'],localCsys=polar)
    ...
    if types[0] in Name: # Displacement controlled
        Model.SmoothStepAmplitude(name='Amp-disp', timeSpan=STEP, data=((0.0, 0.0), (5.0, 5.0)))
        Model.DisplacementBC(name='CavityDisplacement', createStepName='Expansion', region=set_cavity, u1=Dc, u2=UNSET, 
            ur3=UNSET, amplitude='Amp-disp', fixed=OFF, distributionType=UNIFORM, fieldName='', localCsys=polar)
        # Create Field Output
