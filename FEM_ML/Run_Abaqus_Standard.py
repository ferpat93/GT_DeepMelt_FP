...
def f_CreateModel(H,Dc,Hm,Wm,d,E,v,phi,psi,c,Name): # Creates Part from sketch
    ...
    nElRW = 24 # Number elements right wall
    ...
    Assembly.seedEdgeByNumber(edges=edges_cavity, number=36, constraint=FINER) # Cavity
    ...
    # Expansion
    #Model.ImplicitDynamicsStep(name='Expansion', previous='Gravity', timePeriod=10.0, application=QUASI_STATIC, initialInc=0.05, minInc=1e-06, maxInc=0.05, maxNumInc=1000, nohaf=OFF, amplitude=RAMP, alpha=DEFAULT, initialConditions=OFF)
    ...
    timeInterval=0.05, region=set_cavity, sectionPoints=DEFAULT, rebar=EXCLUDE)
