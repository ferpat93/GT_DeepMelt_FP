...
folder = fullfile(pwd,'Data_2000','OutFiles');
sims = GetFiles(folder,'.out');

InputPar = readtable(fullfile(pwd,'Data_2000','InputParameters.txt'),'FileType','text');
InputPar.sv = (9.81/1000).*InputPar.d.*InputPar.Depth;
InputPar.sh = InputPar.sv.*(1-sind(InputPar.phi));

P = GetDelft(InputPar,0.5,5);

figure; histogram(P./InputPar.sv);

for s=1:20%numel(sims)
    data = readmatrix(fullfile(folder,sims{s}),'FileType','text');

    Ps = unique(data(:,1)); % pressures
    par = zeros(numel(Ps),4);
    iv = round(size(data,1)/numel(Ps));
    v = 1:iv:size(data,1);
    
    for si=1:numel(Ps)
        XY = data(v(si):v(si)+(iv-1),2:3); % coords
        [z, a, b, alpha] = fitellipse(XY);
        polygon_area = polyarea([XY(:,1) ;-1.*flipud(XY(:,1))],[XY(:,2); flipud(XY(:,2))]);
        par(si,:) = [Ps(si)/1000 polygon_area 3.1416*a*b min(a,b)/max(a,b)];
%         figure(i)
%         hold on
...
%         xlim([0 1.25]); 
    ...
    ss = sims{s}(1:end-4);
    inp_par = InputPar(strcmp(InputPar.Folder,ss),:);
    
    figure(3); hold on; plot((4/pi).*par(:,2),par(:,1)./inp_par.sv)
    ylabel('Cavity Pressure [kPa]'); xlabel('Cavity Area [m2]')
    %legend('Shallow','Deep')
    figure(4); hold on; plot(par(:,1),par(:,4))
    xlabel('Cavity Pressure [kPa]'); ylabel('Cavity Aspect Ratio [-]')
    legend('Shallow','Deep')
    

    
    figure(5); hold on; plot(1:1:size(par,1),(par(:,1)-inp_par.sh)./(2*inp_par.sv-inp_par.sh))
    ylabel('Cavity Pressure [kPa]'); xlabel('Step')
    
    % When does the cavity yield?
...
function [P] = GetDelft(T,r,c)
    ...
    G = T.E./(2000.*(1+T.v));
    Q = (T.sv .* sen + c.*coss)./G ;
    P = (T.sv.*(1+sen)+c.*(coss+cott)).*((r/Rmax).^2+(Q)).^(-sen./(1+sen)) - c*cott;
