...
%MP = GetPlainStrain(Strain); save('StrainData.mat','MP');
...
%% Loop principal strain orientation

for t=1:size(Strain,1)  
    
    PlotPrincStrainOrientation(PS(t,1:2));
...
if getBands
    ...
    for t=1:size(PS,1)    
        for step=2:2
            ...
            [~,Front_D,~] = get_slices(PS{t,step}(:,:,:,end));  % Deviatoric
...
if getCatenary
    ...
    for t=1:size(PS,1)    
        for step=2:2
            ...
            [~,Front_D,~] = get_slices(PS{t,step}(:,:,:,end));  % Shear
            Front_D(isnan(Front_D))=1;
...
function []=PlotPrincStrainOrientation(V)
